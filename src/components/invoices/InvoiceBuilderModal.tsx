    ...
    <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden text-xs leading-relaxed">
      ...
      <div className="px-5 py-4 grid grid-cols-2 gap-4 border-b border-border">
        <div className="space-y-2">
          ...
          {dueDate && (
            <div>
              <p className="text-light-gray">Jatuh Tempo</p>
      ...
      {metaEntries.length > 0 && (
        <div className="px-5 py-3 border-b border-border bg-subtle-teal/40 space-y-1.5">
          <p className="text-light-gray font-medium mb-2">
...
export function InvoiceBuilderModal({ onClose, onCreated }: Props) {
  ...
  useEffect(() => {
    async function init() {
      ...
      setUserProfile({
        entity_type: profile?.entity_type ?? 'individual', has_npwp: profile?.has_npwp ?? false,
        is_pkp: profile?.is_pkp ?? false, plan, name: profile?.name ?? '', invoice_prefix: profile?.invoice_prefix ?? null,
      })
  ...
  const loadSequence = useCallback(async () => {
    ...
    setNextSeq(seq)
    setInvoiceNumber(`${prefix}-${year}-${String(month).padStart(2,'0')}-${String(seq).padStart(3,'0')}`)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-dark-charcoal/50 backdrop-blur-sm" onClick={onClose} />

      {/* Panel */}
      <div className="relative z-10 w-full sm:max-w-5xl max-h-[95dvh] sm:max-h-[92vh] bg-white sm:rounded-2xl rounded-t-2xl flex flex-col shadow-2xl overflow-hidden">

        {/* ── Modal header ── */}
        <div className="flex-shrink-0 bg-white border-b border-border px-4 lg:px-6 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <h2 className="text-base font-bold text-primary-dark">Invoice Baru</h2>
            {/* Mobile step label */}
            <span className="sm:hidden text-xs text-medium-gray">Langkah {step} dari 5</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setMobileView(v => v === 'form' ? 'preview' : 'form')}
            ...
            {/* Close */}
            <button onClick={onClose} className="p-1.5 text-medium-gray hover:text-primary-dark hover:bg-very-light-gray rounded-lg transition-colors" aria-label="Tutup">
        ...
        {initLoading ? (
          ...
          <div className="flex-1 overflow-hidden">
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] h-full">
              ...
              {/* Form col */}
              <div className={`overflow-y-auto p-5 lg:p-7 ${mobileView === 'preview' ? 'hidden lg:block' : ''}`}>

                {/* STEP 1 – Template */}
                {step === 1 && (
                  <div className="max-w-lg space-y-5">
                    <div>
                      <h3 className="text-lg font-bold text-primary-dark">Pilih Template</h3>
                      <p className="text-sm text-medium-gray mt-1">Template menentukan field tambahan sesuai jenis pekerjaan.</p>
                    </div>
                ...
                {step === 2 && (
                  <div className="max-w-lg space-y-5">
                    <div>
                      <h3 className="text-lg font-bold text-primary-dark">Data Klien</h3>
                ...
                {step === 3 && template && (
                  <div className="max-w-2xl space-y-5">
                    <div>
                      <h3 className="text-lg font-bold text-primary-dark">Item Jasa</h3>
                ...
                {step === 4 && (
                  <div className="max-w-lg space-y-5">
                    <div>
                      <h3 className="text-lg font-bold text-primary-dark">Pratinjau Pajak</h3>
                ...
                {step === 5 && (
                  <div className="max-w-lg space-y-5">
                    <div>
                      <h3 className="text-lg font-bold text-primary-dark">Finalisasi</h3>
                      <p className="text-sm text-medium-gray mt-1">Nomor, tanggal, dan jatuh tempo invoice.</p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      <button type="button" onClick={() => handleSave('draft')} disabled={saving || !invoiceNumber || !issueDate}
                        className="flex-1 py-3 px-5 rounded-xl border-2 border-border text-sm font-semibold text-medium-gray hover:border-primary-teal hover:text-primary-teal transition-all disabled:opacity-50">
                        {saving ? 'Menyimpan…' : 'Simpan sebagai Draft'}
                      </button>
                      <button type="button" onClick={() => handleSave('sent')} disabled={saving || !invoiceNumber || !issueDate}
                        className="flex-1 py-3 px-5 rounded-xl bg-primary-teal text-white text-sm font-semibold hover:bg-primary-teal/90 transition-all shadow-sm disabled:opacity-50">
                ...
                {/* Nav */}
                <div className={`flex items-center ${step === 5 ? 'justify-start' : 'justify-between'} mt-7 max-w-2xl`}>
              ...
              {/* Preview col */}
              <div className={`lg:border-l border-border bg-very-light-gray/60 overflow-y-auto p-4 lg:p-5 ${mobileView === 'form' ? 'hidden lg:block' : ''}`}>
